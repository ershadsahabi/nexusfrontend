...
export default function WorkspaceCanvas({ projectUuid, scenarioId }: { projectUuid: string; scenarioId?: string }) {
  const [rotateEnabled, setRotateEnabled] = useState(false);
  const cameraApiRef = useRef<CameraApi | null>(null);
  const mouseWorld = useCanvasStore((state) => state.mouseWorld);

  const handleToggleOrbit = useCallback(() => setRotateEnabled((prev) => !prev), []);
  ...
  <div className={topBarStyles.group}>
    <RootSystemSelector className={styles.rootSystemSelector} />
  </div>
